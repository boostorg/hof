...
struct construct_f
{
    ...
    T operator()(Ts&&... xs) const BOOST_FIT_NOEXCEPT_CONSTRUCTIBLE(T, Ts&&...)
    {
        storage buffer{};
        new(&buffer) T(BOOST_FIT_FORWARD(Ts)(xs)...);
        storage_holder h(&buffer);
        return h.data();
    }

    template<class X, BOOST_FIT_ENABLE_IF_CONSTRUCTIBLE(T, std::initializer_list<X>&&)>
    T operator()(std::initializer_list<X>&& x) const BOOST_FIT_NOEXCEPT_CONSTRUCTIBLE(T, std::initializer_list<X>&&)
    {
        storage buffer{};
    ...
    T operator()(const std::initializer_list<X>& x) const BOOST_FIT_NOEXCEPT_CONSTRUCTIBLE(T, const std::initializer_list<X>&)
    {
        storage buffer{};
        new(&buffer) T(x);
        storage_holder h(&buffer);
...
template<class T>
struct construct_f<T, typename std::enable_if<BOOST_FIT_IS_LITERAL(T)>::type>
{
    constexpr construct_f() noexcept
    {}
    template<class... Ts, BOOST_FIT_ENABLE_IF_CONSTRUCTIBLE(T, Ts...)>
...
struct construct_template_f
{
    ...
    {}
    template<class... Ts, class Result=BOOST_FIT_JOIN(Template, typename D<Ts>::type...), 
        BOOST_FIT_ENABLE_IF_CONSTRUCTIBLE(Result, Ts...)>
    constexpr Result operator()(Ts&&... xs) const BOOST_FIT_NOEXCEPT_CONSTRUCTIBLE(Result, Ts&&...)
    {
        return construct_f<Result>()(BOOST_FIT_FORWARD(Ts)(xs)...);
    }
};
