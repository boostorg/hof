...
this:
    ...
    BOOST_FIT_STATIC_LAMBDA_FUNCTION(stringify) = conditional(
        [](auto x) BOOST_FIT_RETURNS(std::to_string(x)),
        [](auto x) BOOST_FIT_RETURNS(static_cast<std::ostringstream&>(std::ostringstream() << x).str())
    );

So, using [`BOOST_FIT_RETURNS`](/include/boost/fit/returns) not only deduces the return type for the function, but it also constrains the function on whether the expression is valid or not. So by writing `BOOST_FIT_RETURNS(std::to_string(x))` then the first function will try to call `std::to_string` function if possible. If not, then the second function will be called. 

The second function still uses [`BOOST_FIT_RETURNS`](/include/boost/fit/returns), so the function will still be constrained by whether the `<<` stream operator can be used. Although it may seem unnecessary because there is not another function, however, this makes the function composable. So we could use this to define a `serialize` function that tries to call `stringify` first, otherwise it looks for the member `.serialize()`:

    BOOST_FIT_STATIC_LAMBDA_FUNCTION(serialize) = conditional(
        [](auto x) BOOST_FIT_RETURNS(stringify(x)),
        [](auto x) BOOST_FIT_RETURNS(x.serialize())
    );

static_if
---------

In addition, this can be used with the [`boost::fit::if_`](/include/boost/fit/if) decorator to create `static_if`-like
...
The `id` parameter passed to the lambda is the [`identity`](/include/boost/fit/identity) function. As explained in the article, this is used to delay the lookup of types by making it a dependent type(i.e. the type depends on a template parameter), which is necessary to avoid compile errors. The [`eval`](/include/boost/fit/eval) function that is called will pass this `identity` function to the lambdas.

The advantage of using the Fit library instead of the solution in Baptiste
Wicht's blog, is that [`conditional`](/include/boost/fit/conditional) allows more than just two conditions. So if
