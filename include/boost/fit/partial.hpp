...
#ifdef _MSC_VER
#define BOOST_FIT_PARTIAL_RETURNS(...) -> decltype(__VA_ARGS__) { return (__VA_ARGS__); }
#else
#define BOOST_FIT_PARTIAL_RETURNS BOOST_FIT_SFINAE_RETURNS
#endif
...
struct partial_adaptor_join
{
    ...
    noexcept(BOOST_FIT_IS_NOTHROW_CONSTRUCTIBLE(F, F&&) && boost::fit::pack_join(std::decval<const Pack&>(), boost::fit::pack(BOOST_FIT_FORWARD(Ts)(xs)...)))
#endif
    BOOST_FIT_PARTIAL_RETURNS
    (
        boost::fit::partial
        (
            BOOST_FIT_RETURNS_C_CAST(F&&)(BOOST_FIT_CONST_THIS->get_function(xs...)), 
            boost::fit::pack_join(BOOST_FIT_MANGLE_CAST(const Pack&)(BOOST_FIT_CONST_THIS->get_pack(xs...)), boost::fit::pack(BOOST_FIT_FORWARD(Ts)(xs)...))
        )
    );
};
...
#ifdef _MSC_VER
    // Workaround ICE on MSVC
    noexcept(BOOST_FIT_IS_NOTHROW_CONSTRUCTIBLE(F, F&&) && noexcept(boost::fit::pack(BOOST_FIT_FORWARD(Ts)(xs)...)))
#endif
    BOOST_FIT_PARTIAL_RETURNS
    (
        boost::fit::partial
        (
            BOOST_FIT_RETURNS_C_CAST(F&&)(BOOST_FIT_CONST_THIS->get_function(xs...)), 
            boost::fit::pack(BOOST_FIT_FORWARD(Ts)(xs)...)
        )
    );
};
...
struct partial_adaptor : detail::partial_adaptor_base<F, Pack>::type, F, Pack
{
    ...
    BOOST_FIT_INHERIT_DEFAULT(partial_adaptor, base, F, Pack);

    template<class X, class S>
    constexpr partial_adaptor(X&& x, S&& seq) 
    BOOST_FIT_NOEXCEPT(BOOST_FIT_IS_NOTHROW_CONSTRUCTIBLE(F, X&&) && BOOST_FIT_IS_NOTHROW_CONSTRUCTIBLE(Pack, S&&))
    : F(BOOST_FIT_FORWARD(X)(x)), Pack(BOOST_FIT_FORWARD(S)(seq))
    {}
};
