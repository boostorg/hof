...
#define PLACEHOLDER_CHECK(...) \
    BOOST_FIT_STATIC_TEST_CHECK(__VA_ARGS__); \
    BOOST_FIT_TEST_CHECK(__VA_ARGS__); \
...
BOOST_FIT_TEST_CASE()
{
    auto simple_print = boost::fit::reveal(std::ref(std::cout) << boost::fit::_);
    simple_print("Hello");
}
...
{
    std::stringstream ss;
    auto simple_print = boost::fit::reveal(std::ref(ss) << boost::fit::_);
    simple_print("Hello");
    BOOST_FIT_TEST_CHECK(ss.str() == "Hello");
}
...
BOOST_FIT_TEST_CASE()
{
    ...
    BOOST_FIT_PLACEHOLDER_TEST_CONSTEXPR auto f_not_0 = !boost::fit::_1;
#if defined(__GNUC__) && !defined (__clang__) && __GNUC__ == 4 && __GNUC_MINOR__ > 6   
    static_assert(std::is_copy_constructible<decltype(f_not_0)>::value, "Not copyable");
#endif
    static_assert(boost::fit::detail::is_default_constructible<decltype(f_not_0)>::value, "Not default constructible");
