...
struct static_const_var_factory
{
    ...
    {
        static_assert(BOOST_FIT_IS_DEFAULT_CONSTRUCTIBLE(T), "Static const variable must be default constructible");
...
#define BOOST_FIT_STATIC_AUTO_REF extern __attribute__((weak)) constexpr auto
#else
...
#if defined(__GNUC__) && !defined (__clang__) && __GNUC__ == 4 && __GNUC_MINOR__ < 7
#define BOOST_FIT_STATIC_CONST_VAR(name) extern __attribute__((weak)) constexpr auto name
#else
#define BOOST_FIT_STATIC_CONST_VAR(name) static constexpr auto& name = boost::fit::detail::static_const_var_factory()
#endif

#define BOOST_FIT_DECLARE_STATIC_VAR(name, ...) BOOST_FIT_STATIC_CONST_VAR(name) = __VA_ARGS__{}
