...
/// specified as `BOOST_FIT_ALWAYS_VOID_RETURN`.
/// 
/// Synopsis
/// --------
/// 
...
#if defined(__clang__) && BOOST_FIT_HAS_RELAXED_CONSTEXPR
...
namespace boost { namespace fit { namespace always_detail {
...
struct always_base
{
    ...
    BOOST_FIT_DELGATE_CONSTRUCTOR(always_base, T, x)
    ...
    operator()(As&&...) const
    noexcept(std::is_reference<result_type>::value || BOOST_FIT_IS_NOTHROW_COPY_CONSTRUCTIBLE(result_type))
    {
        return this->x;
    }
};
...
template<class T>
struct always_base<T, typename std::enable_if<!BOOST_FIT_IS_EMPTY(T)>::type>
{
    T x;

    constexpr always_base(T xp) noexcept(BOOST_FIT_IS_NOTHROW_COPY_CONSTRUCTIBLE(T))
    : x(xp)
    {}
...
#define BOOST_FIT_ALWAYS_VOID_RETURN boost::fit::always_detail::always_base<void>::void_
#else
...
{
    template<class T>
    constexpr always_detail::always_base<T> operator()(T x) const noexcept(BOOST_FIT_IS_NOTHROW_COPY_CONSTRUCTIBLE(T))
    {
...
BOOST_FIT_DECLARE_STATIC_VAR(always, always_detail::always_f);
BOOST_FIT_DECLARE_STATIC_VAR(always_ref, always_detail::always_ref_f);
