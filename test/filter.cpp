...
struct integer_predicate
{
    ...
    (
        boost::fit::conditional(
            boost::fit::if_(std::is_integral<T>())(boost::fit::pack_basic),
            boost::fit::always(boost::fit::pack_basic())
        )(boost::fit::move(x))
    )
};
...
BOOST_FIT_TEST_CASE()
{
    BOOST_FIT_TEST_CHECK(filter_integers()(boost::fit::pack_basic(1, 2, 2.0, 3)) == std::make_tuple(1, 2, 3));
#if BOOST_FIT_HAS_CONSTEXPR_TUPLE
    BOOST_FIT_STATIC_TEST_CHECK(filter_integers()(boost::fit::pack_basic(1, 2, 2.0, 3)) == std::make_tuple(1, 2, 3));
#endif
}
