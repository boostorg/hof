...
Say, for example, we would like to write a print function. We could start by writing the function that prints using `std::cout`, like this:
    ...
    BOOST_FIT_STATIC_LAMBDA_FUNCTION(print) = [](const auto& x)
    {
        std::cout << x << std::endl;
    };
...
The Fit library provides several ways to do overloading. One of the ways is with the [`conditional`](/include/boost/fit/conditional) adaptor which will pick the first function that is callable. This allows ordering the functions based on which one is more important. So then the first function will print to `std::cout` if possible otherwise we will add an overload to print a range:
...
We could extend this to printing tuples as well. We will need to combine a couple of functions to make a `for_each_tuple`, which lets us call a function for each element. First, the [`by`](/include/boost/fit/by) adaptor will let us apply a function to each argument passed in, and the [`unpack`](/include/boost/fit/unpack) adaptor will unpack the elements of a tuple and apply them to the function:
...
We can also make this `print` function variadic, so it prints every argument passed into it. We can use the [`by`](/include/boost/fit/by) adaptor, which already calls the function on every argument passed in. First, we just rename our original `print` function to `simple_print`:
...
And then apply the [`by`](/include/boost/fit/by) adaptor to `simple_print`:
