...
BOOST_FIT_TEST_CASE()
{
    boost::fit::by(boost::fit::identity, boost::fit::identity)(0);
}
...
BOOST_FIT_TEST_CASE()
{
    auto f = boost::fit::by(bar{});
    static_assert(!boost::fit::is_callable<decltype(f), int>::value, "Not sfinae friendly");
    static_assert(!boost::fit::is_callable<decltype(f), int, int>::value, "Not sfinae friendly");
    static_assert(!boost::fit::is_callable<decltype(f), int, int, int>::value, "Not sfinae friendly");
}

BOOST_FIT_TEST_CASE()
{
    auto f = boost::fit::by(bar{}, bar{});
    static_assert(!boost::fit::is_callable<decltype(f), int>::value, "Not sfinae friendly");
    static_assert(!boost::fit::is_callable<decltype(f), int, int>::value, "Not sfinae friendly");
    static_assert(!boost::fit::is_callable<decltype(f), int, int, int>::value, "Not sfinae friendly");
    static_assert(!boost::fit::is_callable<decltype(f)>::value, "Not sfinae friendly");
}
