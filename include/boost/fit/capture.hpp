...
struct capture_invoke : detail::compressed_pair<detail::callable_base<F>, Pack>, detail::function_result_type<F>
{
    ...
    BOOST_FIT_INHERIT_CONSTRUCTOR(capture_invoke, base)
    template<class... Ts>
    ...
    (
        capture_invoke<F, Pack>(BOOST_FIT_RETURNS_STATIC_CAST(F&&)(f), 
            BOOST_FIT_RETURNS_C_CAST(Pack&&)(
                BOOST_FIT_RETURNS_STATIC_CAST(const Pack&)(*boost::fit::always(BOOST_FIT_CONST_THIS)(f))
            )
        )
    );
};
...
struct make_capture_pack_f
{
    ...
    BOOST_FIT_NOEXCEPT_CONSTRUCTIBLE(capture_pack<Pack>, Pack&&)
    {
...
BOOST_FIT_DECLARE_STATIC_VAR(capture_basic, detail::capture_f<detail::pack_basic_f>);
