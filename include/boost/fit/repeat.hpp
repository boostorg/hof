...
struct repeater<0>
{
    ...
    BOOST_FIT_RETURNS_DEDUCE_NOEXCEPT(T(x))
    {
        return x;
    }
};
...
struct repeat_integral_decorator<0>
{
    ...
    BOOST_FIT_RETURNS_DEDUCE_NOEXCEPT((n--, f(BOOST_FIT_FORWARD(T)(x))))
    -> decltype(f(BOOST_FIT_FORWARD(T)(x)))
    {
        while(n > 0)
        {
            n--;
            x = f(BOOST_FIT_FORWARD(T)(x));
        }
        return x;
    }
...
#if BOOST_FIT_HAS_RELAXED_CONSTEXPR
#define BOOST_FIT_REPEAT_CONSTEXPR_DEPTH 1
#else
#define BOOST_FIT_REPEAT_CONSTEXPR_DEPTH BOOST_FIT_RECURSIVE_CONSTEXPR_DEPTH
#endif

BOOST_FIT_DECLARE_STATIC_VAR(repeat, decorate_adaptor<
    boost::fit::conditional_adaptor<
    detail::repeat_constant_decorator, 
    detail::repeat_integral_decorator<BOOST_FIT_REPEAT_CONSTEXPR_DEPTH>
>>);
