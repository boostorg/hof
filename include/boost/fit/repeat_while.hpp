...
struct repeat_while_integral_decorator<0>
{
    ...
    BOOST_FIT_RETURNS_DEDUCE_NOEXCEPT((p(x), f(x)))
    -> decltype(f(x))
    {
        while(p(x))
        {
...
#if BOOST_FIT_HAS_RELAXED_CONSTEXPR
#define BOOST_FIT_REPEAT_WHILE_CONSTEXPR_DEPTH 1
#else
#define BOOST_FIT_REPEAT_WHILE_CONSTEXPR_DEPTH BOOST_FIT_RECURSIVE_CONSTEXPR_DEPTH
#endif
...
BOOST_FIT_DECLARE_STATIC_VAR(repeat_while, decorate_adaptor<
    boost::fit::conditional_adaptor<
        ...
        detail::repeat_while_integral_decorator<BOOST_FIT_REPEAT_WHILE_CONSTEXPR_DEPTH>
    >
>);
