...
struct implicit
{
    ...
    struct invoker
    {
        ...
        constexpr invoker(Pack pp) BOOST_FIT_NOEXCEPT_CONSTRUCTIBLE(Pack, Pack&&)
        : p(boost::fit::move(pp))
        {}

        template<class X, class=typename std::enable_if<detail::is_implicit_callable<F<X>, Pack, X>::value>::type>
        constexpr operator X() const BOOST_FIT_NOEXCEPT(noexcept(p(F<X>())))
        {
            return p(F<X>());
        }
    ...
    {
        template<class Pack>
        constexpr invoker<Pack> operator()(Pack p) const BOOST_FIT_NOEXCEPT(noexcept(invoker<Pack>(boost::fit::move(p))))
        {
            return invoker<Pack>(boost::fit::move(p));
        }
    ...
    BOOST_FIT_RETURNS
    (
        BOOST_FIT_RETURNS_CONSTRUCT(make_invoker)()(boost::fit::pack_basic(BOOST_FIT_FORWARD(Ts)(xs)...))
    );
};
