...
constexpr T ref alias_value(alias_inherit<T, Tag> ref a, Ts&&...) BOOST_FIT_RETURNS_DEDUCE_NOEXCEPT(move(a)) \
{ \
    return move(a); \
}
...
: std::conditional<(BOOST_FIT_IS_CLASS(T) && !BOOST_FIT_IS_FINAL(T) && !BOOST_FIT_IS_POLYMORPHIC(T)), 
    alias_inherit<T, Tag>, 
    alias<T, Tag>
>
{};

#if BOOST_FIT_HAS_EBO
template<class T, class Tag>
struct alias_empty
