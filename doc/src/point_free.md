...
Instead with point-free style, we can write this using the [`by`](/include/boost/fit/by) adaptor, which calls a function on each arguments. Of course, `std::cout` is not a function, but we can make it one by using `BOOST_FIT_LIFT`:

    BOOST_FIT_STATIC_FUNCTION(simple_print) = BOOST_FIT_LIFT(std::ref(std::cout) << _);

This uses the [placeholders](/include/boost/fit/placeholders) to create a function that prints to `std::cout`. Then we can pass `simple_print` to the [`by`](/include/boost/fit/by) adaptor:

    BOOST_FIT_STATIC_FUNCTION(print) = by(simple_print);

As the [`by`](/include/boost/fit/by) adaptor calls the function for each argument passed in, `b(f)(x, y)` is the equivalent of calling `f(x)` and then `f(y)`. In this case, it will call `simple_print(x)` and then `simple_print(y)`:
...
With point-free style programming, we can recognize this is a [fold](https://en.wikipedia.org/wiki/Fold_%28higher-order_function%29), and write it using the [`fold`](/include/boost/fit/fold) adaptor, which will do a fold over the variadic parameters:

    BOOST_FIT_STATIC_FUNCTION(max) = fold(BOOST_FIT_LIFT(std::max));

[`BOOST_FIT_LIFT`](/include/boost/fit/lift) is used to grab the entire overload set of `std::max` function, which is needed since `std::max` is templated and we want the variadic `std::max` function to handle any types as well. So now it can be called like this:

    auto n = max(1, 2, 4, 3); // Returns 4
    auto m = max(0.1, 0.2, 0.5, 0.4); // Returns 0.5

By using [`fold`](/include/boost/fit/fold), `max(1, 2, 4, 3)` will call `std::max` like `std::max(std::max(std::max(1, 2), 4), 3)` and `max(0.1, 0.2, 0.5, 0.4)` will be called like `std::max(std::max(std::max(0.1, 0.2), 0.5), 0.4)`.
