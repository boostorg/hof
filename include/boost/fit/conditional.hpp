...
{
    BOOST_FIT_INHERIT_DEFAULT(basic_conditional_adaptor, F1, F2)
    ...
    noexcept(BOOST_FIT_IS_NOTHROW_CONSTRUCTIBLE(F1, A&&) && BOOST_FIT_IS_NOTHROW_CONSTRUCTIBLE(F2, B&&))
    : F1(BOOST_FIT_FORWARD(A)(f1)), F2(BOOST_FIT_FORWARD(B)(f2))
    {}
    ...
    BOOST_FIT_NOEXCEPT_CONSTRUCTIBLE(F1, X&&)
    : F1(BOOST_FIT_FORWARD(X)(x))
    {} 
    ...
    BOOST_FIT_RETURNS_CLASS(basic_conditional_adaptor);
    ...
    (
        BOOST_FIT_RETURNS_STATIC_CAST(const F&)(*BOOST_FIT_CONST_THIS)(BOOST_FIT_FORWARD(Ts)(xs)...)
    );
};
...
struct conditional_kernel : compressed_pair<F1, F2>
{
    ...
    BOOST_FIT_INHERIT_CONSTRUCTOR(conditional_kernel, base)
    ...
    BOOST_FIT_SFINAE_RETURNS
    (
        boost::fit::detail::which(
            BOOST_FIT_RETURNS_CONSTRUCT(PickFirst)(),
            BOOST_FIT_MANGLE_CAST(const F1&)(BOOST_FIT_CONST_THIS->first(xs...)),
            BOOST_FIT_MANGLE_CAST(const F2&)(BOOST_FIT_CONST_THIS->second(xs...))
        )
...
: detail::conditional_kernel<F, BOOST_FIT_JOIN(conditional_adaptor, Fs...) >
{
    ...
    noexcept(BOOST_FIT_IS_NOTHROW_CONSTRUCTIBLE(base, X&&, kernel_base) && BOOST_FIT_IS_NOTHROW_CONSTRUCTIBLE(kernel_base, Xs&&...))
    ...
    BOOST_FIT_NOEXCEPT_CONSTRUCTIBLE(base, X&&)
